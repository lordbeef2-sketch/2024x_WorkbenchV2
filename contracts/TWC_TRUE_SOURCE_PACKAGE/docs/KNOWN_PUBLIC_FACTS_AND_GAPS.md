...
4. Teamwork Cloud authentication server supports **SAML**, and in that integration the authentication server acts as a **Service Provider**.
5. Teamwork Cloud docs describe **token-based authentication** for REST and warn that sessions remain open unless explicitly reused/logged out.
6. Teamwork Cloud docs describe model manipulation at the **EMF level**, with MagicDraw-specific extensions available to reduce call count for common model tasks.
7. 2024xR3 release notes mention **improvements to SSO support across command-line tools** and new publish templates for Cameo Collaborator documents.
...
2. Exact session/cookie/token behavior after SAML in your environment.
